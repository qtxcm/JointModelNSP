...
//Joint Model For Segmentation, POS and Normalization 
public class SNT {
	 ...
	 * @param lmWord_file
	 */
	public SNT(String test_file, String model_file, String output_file,
			String evaluationError_file, int search_width, String sense_file,
			String out_path, String lm_file, String lmWord_file) {
	...
	public SNT(String train_file, int number_of_train, String model_file,
			...
			String log_file, String lmChar_file, String lmWord_file) {
		this.train_file = train_file;
		this.number_of_train = number_of_train;
		this.model_file = model_file;
		this.number_of_iterations = number_of_iterations;
	...
	public void trainDevTestProcess() throws Exception {
		...
		for (int n = 0; n < number_of_iterations; n++) {
			...
			System.out.println("Correct Intance Num: "
					+ Integer.toString(curTrainIterCorrectInstance));
			...
			save(this.arrTestResult, output_path + this.output_file + n);
			eva = new Evaluator(this.arrTestResult, arrTestSource, bwlog,
	...
	public void trainProcess() throws Exception {
		...
		for (int n = 0; n < number_of_iterations; n++) {
			...
			for (int i = 0; i < arrTrainSource.size(); i++) {
				...
				// this.arrCurSentecFeature =
				// GetFeatureBySentence(this.arrTrainSource[i]);
				this.CurSentence = UnTagSentence(this.arrTrainSource.get(i));
				this.goldActions = getGoldActions(this.arrTrainSource.get(i));
				trainer(n, i);
			}
	...
	public void trainer(int round, int sentenceIndex) throws Exception {
		...
		System.out.print("setnece:" + sentenceIndex + " begin:");
		System.out.flush();
		long st1 = System.nanoTime();
		curRoundIndexForTrain++;
		for (int i = 0; i <= this.CurSentence.length(); i++) {
			String curSChar = "";
			if (i < this.CurSentence.length())
				curSChar = String.valueOf(CurSentence.charAt(i));		
			int agendaLen = 1;
			if (i == 0)
				agendaLen = 1;
			else
				...
				if (i == this.CurSentence.length()) {
					if (arrSenseSet.size() > 0) {
						for (int m = 0; m < arrSenseSet.size(); m++) {
							State temCand = Finish(state, null, false,
									arrSenseSet.get(m));// end action
							if (temCand.score >= temAgenda[0].score) {
								if (state.bIsGold == true
										&& this.goldActions.get(i) == 2000) {
									if (temCand.size >= 1) {
										if (equalNormal(temCand,
												this.goldSentenceState, 1) == true) {
			...
			if (bEqual == false)
			{
				...
				System.out.println(String.format(
						"Update at %d/%d, best score %f, gold score %f",
						predActions.size(), goldActions.size(), scores[0],
						scores[1]));
				return;
			}
		}
		...
		if (this.agenda[0].bIsGold == false) {
			...
			System.out.println(String.format(
					"Update at end: %d/%d, best score %f, gold score %f",
					predActions.size(), goldActions.size(), scores[0],
					scores[1]));
		} else {
			this.curTrainIterCorrectInstance++;
			System.out.println(String.format("Corrected : %d",
					goldActions.size()));
		}		
	}
	...
	public double GetLocalFeaturesScore(State state, List<String> fvs,
		...
		if (size > 0) {
			...
			if (normalSent.length() > 3)
				normalSent.substring(normalSent.length() - 3,
						normalSent.length());
			normalSent = normalSent + n_1;
			...
			if (state.lastAction == 1000) {// app
				...
				// if(charCat0 !=null && charCat0.indexOf(t_0+"|")>-1 &&
				// charCat1 !=null && charCat1.indexOf(t_0+"|")>-1){
				...
				/*
				 * strfeat = "OrgSeenWords=" + w_1; if(fvs !=
				 * null)fvs.add(strfeat); fe =
				 * model.m_mapOrgSeenWords.get(strfeat); if (fe != null) dScore
				 * += bAverage ? fe.aveWeight : fe.weight;
				 * 
				 * strfeat = "OrgLastWordByWord=" + w_1 + "_" + w_2; if(fvs !=
				 * null)fvs.add(strfeat); fe =
				 * model.m_mapOrgLastWordByWord.get(strfeat); if (fe != null)
				 * dScore += bAverage ? fe.aveWeight : fe.weight;
				 * 
				 * 
				 * if(len_w_1 == 1){ strfeat = "OrgOneCharWord=" + w_1; if(fvs
				 * != null)fvs.add(strfeat); fe =
				 * model.m_mapOrgOneCharWord.get(strfeat); if (fe != null)
				 * dScore += bAverage ? fe.aveWeight : fe.weight;
				 * 
				 * strfeat = "OrgTagOfOneCharWord=" + c_2 + c_1 + c_0 + t_1;
				 * if(fvs != null)fvs.add(strfeat); fe =
				 * model.m_mapOrgTagOfOneCharWord.get(strfeat); if (fe != null)
				 * dScore += bAverage ? fe.aveWeight : fe.weight; }
				 * 
				 * strfeat = "OrgFirstAndLastChars=" + start_w_1 + end_w_1;
				 * if(fvs != null)fvs.add(strfeat); fe =
				 * model.m_mapOrgFirstAndLastChars.get(strfeat); if (fe != null)
				 * dScore += bAverage ? fe.aveWeight : fe.weight;
				 * 
				 * strfeat = "OrgLengthByLastChar=" + end_w_1 + len_w_1; if(fvs
				 * != null)fvs.add(strfeat); fe =
				 * model.m_mapOrgLengthByLastChar.get(strfeat); if (fe != null)
				 * dScore += bAverage ? fe.aveWeight : fe.weight;
				 * 
				 * strfeat = "OrgLengthByFirstChar=" + start_w_1 + len_w_1;
				 * if(fvs != null)fvs.add(strfeat); fe =
				 * model.m_mapOrgLengthByFirstChar.get(strfeat); if (fe != null)
				 * dScore += bAverage ? fe.aveWeight : fe.weight;
				 * 
				 * strfeat = "OrgCurrentWordLastChar=" + end_w_2 + "_" + w_1;
				 * if(fvs != null)fvs.add(strfeat); fe =
				 * model.m_mapOrgCurrentWordLastChar.get(strfeat); if (fe !=
				 * null) dScore += bAverage ? fe.aveWeight : fe.weight;
				 * 
				 * strfeat = "OrgLastWordByLastChar=" + end_w_2 + end_w_1;
				 * if(fvs != null)fvs.add(strfeat); fe =
				 * model.m_mapOrgLastWordByLastChar.get(strfeat); if (fe !=
				 * null) dScore += bAverage ? fe.aveWeight : fe.weight;
				 * 
				 * strfeat = "OrgLengthByLastWord=" + w_2 + len_w_1; if(fvs !=
				 * null)fvs.add(strfeat); fe =
				 * model.m_mapOrgLengthByLastWord.get(strfeat); if (fe != null)
				 * dScore += bAverage ? fe.aveWeight : fe.weight;
				 * 
				 * strfeat = "OrgLastLengthByWord=" + len_w_2 + w_1; if(fvs !=
				 * null)fvs.add(strfeat); fe =
				 * model.m_mapOrgLastLengthByWord.get(strfeat); if (fe != null)
				 * dScore += bAverage ? fe.aveWeight : fe.weight;
				 * 
				 * strfeat = "OrgCurrentTag=" + w_1 + t_1; if(fvs !=
				 * null)fvs.add(strfeat); fe =
				 * model.m_mapOrgCurrentTag.get(strfeat); if (fe != null) dScore
				 * += bAverage ? fe.aveWeight : fe.weight;
				 * 
				 * //if(len_w_1<=2){ strfeat = "OrgTagByWordAndPrevChar=" + w_1
				 * + t_1 + end_w_2; if(fvs != null)fvs.add(strfeat); fe =
				 * model.m_mapOrgTagByWordAndPrevChar.get(strfeat); if (fe !=
				 * null) dScore += bAverage ? fe.aveWeight : fe.weight;
				 * 
				 * strfeat = "OrgTagByLastWord=" + w_1 + t_0; if(fvs !=
				 * null)fvs.add(strfeat); fe =
				 * model.m_mapOrgTagByLastWord.get(strfeat); if (fe != null)
				 * dScore += bAverage ? fe.aveWeight : fe.weight;
				 * 
				 * strfeat = "OrgTagByWordAndNextChar=" + w_1 + t_1 + c_0;
				 * if(fvs != null)fvs.add(strfeat); fe =
				 * model.m_mapOrgTagByWordAndNextChar.get(strfeat); if (fe !=
				 * null) dScore += bAverage ? fe.aveWeight : fe.weight; //}
				 * 
				 * strfeat = "OrgLastTagByWord=" + w_1 + t_2; if(fvs !=
				 * null)fvs.add(strfeat); fe =
				 * model.m_mapOrgLastTagByWord.get(strfeat); if (fe != null)
				 * dScore += bAverage ? fe.aveWeight : fe.weight;
				 * 
				 * strfeat = "OrgTagByLastChar=" + end_w_1 + t_1; if(fvs !=
				 * null)fvs.add(strfeat); fe =
				 * model.m_mapOrgTagByLastChar.get(strfeat); if (fe != null)
				 * dScore += bAverage ? fe.aveWeight : fe.weight;
				 * 
				 * strfeat = "OrgLastTagByTag=" + t_1 + t_0; if(fvs !=
				 * null)fvs.add(strfeat); fe =
				 * model.m_mapOrgLastTagByTag.get(strfeat); if (fe != null)
				 * dScore += bAverage ? fe.aveWeight : fe.weight;
				 * 
				 * strfeat = "OrgTag0Tag1Size1=" + t_1 + t_0 + len_w_1; if(fvs
				 * != null)fvs.add(strfeat); fe =
				 * model.m_mapOrgTag0Tag1Size1.get(strfeat); if (fe != null)
				 * dScore += bAverage ? fe.aveWeight : fe.weight;
				 * 
				 * strfeat = "OrgTag1Tag2Size1=" + t_2 + t_1 + len_w_1; if(fvs
				 * != null)fvs.add(strfeat); fe =
				 * model.m_mapOrgTag1Tag2Size1.get(strfeat); if (fe != null)
				 * dScore += bAverage ? fe.aveWeight : fe.weight;
				 * 
				 * strfeat = "OrgTag0Tag1Tag2Size1=" + t_2 + t_1 + t_0 +
				 * len_w_1; if(fvs != null)fvs.add(strfeat); fe =
				 * model.m_mapOrgTag0Tag1Tag2Size1.get(strfeat); if (fe != null)
				 * dScore += bAverage ? fe.aveWeight : fe.weight;
				 * 
				 * strfeat = "OrgLastTwoTagsByTag=" + t_2 + t_1 + t_0; if(fvs !=
				 * null)fvs.add(strfeat); fe =
				 * model.m_mapOrgLastTwoTagsByTag.get(strfeat); if (fe != null)
				 * dScore += bAverage ? fe.aveWeight : fe.weight;
				 * 
				 * strfeat = "OrgTagByChar=" + t_0 + c_0; if(fvs !=
				 * null)fvs.add(strfeat); fe =
				 * model.m_mapOrgTagByFirstChar.get(strfeat); if (fe != null)
				 * dScore += bAverage ? fe.aveWeight : fe.weight;
				 * 
				 * strfeat = "OrgFirstCharBy2Tags=" + t_0 + t_1 + c_0; if(fvs !=
				 * null)fvs.add(strfeat); fe =
				 * model.m_mapOrgFirstCharBy2Tags.get(strfeat); if (fe != null)
				 * dScore += bAverage ? fe.aveWeight : fe.weight;
				 * 
				 * strfeat = "OrgSeparateChars=" + c_1 + c_0; if(fvs !=
				 * null)fvs.add(strfeat); fe =
				 * model.m_mapOrgSeparateChars.get(strfeat); if (fe != null)
				 * dScore += bAverage ? fe.aveWeight : fe.weight;
				 * 
				 * strfeat = "OrgLastWordFirstChar=" + w_1 + "_" + c_0; if(fvs
				 * != null)fvs.add(strfeat); fe =
				 * model.m_mapOrgLastWordFirstChar.get(strfeat); if (fe != null)
				 * dScore += bAverage ? fe.aveWeight : fe.weight;
				 * 
				 * strfeat = "OrgFirstCharLastWordByWord=" + start_w_1 + c_0;
				 * if(fvs != null)fvs.add(strfeat); fe =
				 * model.m_mapOrgFirstCharLastWordByWord.get(strfeat); if (fe !=
				 * null) dScore += bAverage ? fe.aveWeight : fe.weight;
				 * 
				 * strfeat = "OrgTaggedSeparateChars=" + end_w_1 + t_1 + c_0 +
				 * t_0; if(fvs != null)fvs.add(strfeat); fe =
				 * model.m_mapOrgTaggedSeparateChars.get(strfeat); if (fe !=
				 * null) dScore += bAverage ? fe.aveWeight : fe.weight;
				 * 
				 * strfeat = "OrgTagWordTag=" + t_2 + w_1 + t_0; if(fvs !=
				 * null)fvs.add(strfeat); fe =
				 * model.m_mapOrgTagWordTag.get(strfeat); if (fe != null) dScore
				 * += bAverage ? fe.aveWeight : fe.weight;
				 * 
				 * strfeat = "OrgWordTagTag=" + w_2 + t_1 + t_0; if(fvs !=
				 * null)fvs.add(strfeat); fe =
				 * model.m_mapOrgWordTagTag.get(strfeat); if (fe != null) dScore
				 * += bAverage ? fe.aveWeight : fe.weight;
				 * 
				 * strfeat = "OrgTagByFirstChar=" + start_w_1 + t_1; if(fvs !=
				 * null)fvs.add(strfeat); fe =
				 * model.m_mapOrgTagByFirstChar.get(strfeat); if (fe != null)
				 * dScore += bAverage ? fe.aveWeight : fe.weight;
				 * 
				 * for (int j = 0; j < len_w_1 - 1; ++j) { strfeat =
				 * "OrgTaggedCharByLastChar=" + w_1.substring(j, j + 1) + t_1 +
				 * end_w_1; if(fvs != null)fvs.add(strfeat); fe =
				 * model.m_mapOrgTaggedCharByLastChar.get(strfeat); if (fe !=
				 * null) dScore += bAverage ? fe.aveWeight : fe.weight; }
				 */
				...
				if (len_n_1 == 1) {
					...
					if (fe != null)
						dScore += bAverage ? fe.aveWeight * 1.0
								: fe.weight * 1.0;
				...
				for (int j = 0; j < len_n_1 - 1; ++j) {
					strfeat = "NorTaggedCharByLastChar="
							+ n_1.substring(j, j + 1) + t_1 + end_n_1;
					if (fvs != null)
						fvs.add(strfeat);
				...
				// char-based language model feature
				...
				// word-based language model feature
				if (n_1.length() > 0) {
					String words = n_1;
				...
				/*
				 * if(n_1!=null && n_1.length()>0){ strfeat = "WordSense=" + w_1
				 * + n_1; if(fvs != null)fvs.add(strfeat); fe =
				 * model.m_mapWordSense.get(strfeat); if (fe != null) dScore +=
				 * bAverage ? fe.aveWeight : fe.weight;
				 * 
				 * strfeat = "LastWordAndWordSense=" + n_1+w_0; if(fvs !=
				 * null)fvs.add(strfeat); fe =
				 * model.m_mapLastWordAndWordSense.get(strfeat); if (fe != null)
				 * dScore += bAverage ? fe.aveWeight : fe.weight;
				 * 
				 * strfeat = "PreWordAndWordSense=" + n_1+w_2; if(fvs !=
				 * null)fvs.add(strfeat); fe =
				 * model.m_mapPreWordAndWordSense.get(strfeat); if (fe != null)
				 * dScore += bAverage ? fe.aveWeight : fe.weight;
				 * 
				 * strfeat = "EndPreAndWordSense=" + n_1+end_w_2; if(fvs !=
				 * null)fvs.add(strfeat); fe =
				 * model.m_mapEndPreAndWordSense.get(strfeat); if (fe != null)
				 * dScore += bAverage ? fe.aveWeight : fe.weight;
				 * 
				 * 
				 * strfeat = "LastCharAndWordSense=" + n_1 + c_0; if(fvs !=
				 * null)fvs.add(strfeat); fe =
				 * model.m_mapLastCharAndWordSense.get(strfeat); if (fe != null)
				 * dScore += bAverage ? fe.aveWeight : fe.weight;
				 * 
				 * strfeat = "TagWordSense=" + n_1 + t_1; if(fvs !=
				 * null)fvs.add(strfeat); fe =
				 * model.m_mapTagWordSense.get(strfeat); if (fe != null) dScore
				 * += bAverage ? fe.aveWeight : fe.weight;
				 * 
				 * strfeat = "PreTagAndWordSense=" + n_1 + t_2; if(fvs !=
				 * null)fvs.add(strfeat); fe =
				 * model.m_mapPreTagAndWordSense.get(strfeat); if (fe != null)
				 * dScore += bAverage ? fe.aveWeight : fe.weight;
				 * 
				 * strfeat = "LastTagAndWordSense=" + n_1 + t_0; if(fvs !=
				 * null)fvs.add(strfeat); fe =
				 * model.m_mapLastTagAndWordSense.get(strfeat); if (fe != null)
				 * dScore += bAverage ? fe.aveWeight : fe.weight;
				 * 
				 * strfeat = "ThreeWordAndSense=" + n_1+w_0+w_2; if(fvs !=
				 * null)fvs.add(strfeat); fe =
				 * model.m_mapThreeWordAndSense.get(strfeat); if (fe != null)
				 * dScore += bAverage ? fe.aveWeight : fe.weight;
				 * 
				 * strfeat = "PreLastTagAndWordSense=" + n_1+t_0+t_2; if(fvs !=
				 * null)fvs.add(strfeat); fe =
				 * model.m_mapPreLastTagAndWordSense.get(strfeat); if (fe !=
				 * null) dScore += bAverage ? fe.aveWeight : fe.weight;
				 * 
				 * if(n_2!=null && n_2.length()>0){
				 * 
				 * strfeat = "TwoWordSense=" + n_1+ n_2; if(fvs !=
				 * null)fvs.add(strfeat); fe =
				 * model.m_mapTwoWordSense.get(strfeat); if (fe != null) dScore
				 * += bAverage ? fe.aveWeight : fe.weight;
				 * 
				 * strfeat = "LastTagAndTwoWordSense=" + n_1+ n_2 + t_0; if(fvs
				 * != null)fvs.add(strfeat); fe =
				 * model.m_mapLastTagAndTwoWordSense.get(strfeat); if (fe !=
				 * null) dScore += bAverage ? fe.aveWeight : fe.weight;
				 * 
				 * strfeat = "LastWordAndTwoWordSense=" + n_1+ n_2 + w_0; if(fvs
				 * != null)fvs.add(strfeat); fe =
				 * model.m_mapLastWordAndTwoWordSense.get(strfeat); if (fe !=
				 * null) dScore += bAverage ? fe.aveWeight : fe.weight;
				 * 
				 * } }
				 */
				...
				/*
				 * if(n_2!=null && n_2.length()>0){ strfeat =
				 * "LastTagAndPreWordSense=" + n_2 + t_0; if(fvs !=
				 * null)fvs.add(strfeat); fe =
				 * model.m_mapLastTagAndPreWordSense.get(strfeat); if (fe !=
				 * null) dScore += bAverage ? fe.aveWeight : fe.weight;
				 * 
				 * strfeat = "LastWordAndPreWordSense=" + n_2 + w_0; if(fvs !=
				 * null)fvs.add(strfeat); fe =
				 * model.m_mapLastWordAndPreWordSense.get(strfeat); if (fe !=
				 * null) dScore += bAverage ? fe.aveWeight : fe.weight;
				 * 
				 * strfeat = "LastCharAndPreWordSense=" + n_2 + c_0; if(fvs !=
				 * null)fvs.add(strfeat); fe =
				 * model.m_mapLastCharAndPreWordSense.get(strfeat); if (fe !=
				 * null) dScore += bAverage ?fe.aveWeight : fe.weight; }
				 */
	...
	public double[] updateParameters(List<Integer> predActions, State bestState) {
		...
		for (; p < predActions.size(); p++) {
			...
			if (p < this.CurSentence.length())
				curSChar = String.valueOf(CurSentence.charAt(p));
		...
		for (; p < predActions.size(); p++) {
			...
			currentScore = GetLocalFeaturesScore(tmpState, predFeatures,
					false);
			...
			currentScore = GetLocalFeaturesScore(goldState, goldFeatures,
					false);
			scores[1] += currentScore;
		...
		if (this.lmChar_file != null && this.lmChar_file.length() > 0) {
			...
			try {
				...
				BufferedReader reader = new BufferedReader(
						new InputStreamReader(fis, "UTF8"));// 用50M的缓冲读取文本文件
				String line = "";
